...
const Anecdote = ({ anec }) => {
  return (
    <div>
      <h2>{anec.content} <i>by</i> {anec.author}</h2>
...
const CreateNew = (props) => {
  ...
  const { clear: clearC, ...content } = useField('text')
  const { clear: clearA, ...author} = useField('text')
  const { clear: clearI, ...info} = useField('text')
